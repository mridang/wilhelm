...
// generator.
func runClientsetMode(pkgName string) (string, string, error) {
	...
	rendered, err := emitClientsetFile(pkgName, resources)
	if err != nil {
	...
	summary := fmt.Sprintf("clientset mode: %d resources (%d with assertions)",
		len(resources), withAssert)
...
// clientset-introspection mode.
func emitClientsetFile(pkgName string, resources []resource) (string, error) {
	...
	emitClientsetAssertPartial(f, assertResources)
	emitClientsetAssertNone(f, assertResources)
...
// resource, and runs assert.Partial.
func emitClientsetAssertPartial(f *jen.File, resources []resource) {
...
// resource by name and asserts the API returned NotFound.
func emitClientsetAssertNone(f *jen.File, resources []resource) {
